...
from helpers.streamrip_config import set_qobuz_quality
...
class Qdl(commands.Cog, name="qdl"):
    ...
    @commands.command(name="qdl")
    @commands.has_any_role("Hi-Res Downloader")
    async def dl(self, ctx, link, quality):
        """
        Downloads music from Qobuz.
        """
        ...
        if "qobuz" not in link:
            await ctx.reply(f"{ctx.author.mention}, use `h!qdl` only for Qobuz links.")
            return
        ...
        if "https" not in link or ".com" not in link:
            await ctx.reply(f"Please send a valid `https://` Qobuz link, {ctx.author.mention}.")
            return
        ...
        try:
            ...
            archive_name = artifact_name_from_temp(temp_path, fallback="Qobuz request")
